...
import pandas as pd
from src.eda import eda    ### Easy Data Augmentation by Jason W. Wei
...
def tokenize_multipart_input(
    ...
    if 'BertTokenizer' in type(tokenizer).__name__:   #### edited from 'BERT' --> 'BertTokenizer'
        # Only provide token type ids for BERT
        result['token_type_ids'] = token_type_ids
    else:
        result['token_type_ids'] = None   ###### added
...
class FewShotDataset(torch.utils.data.Dataset):
    ...
    def new_convert_fn(
        ...
        if not use_demo:
            ...
            ######## For Easy Data Augmentation
            # eda_augmented_example = [eda(example) for example in augmented_example]
            #
            # eda_inputs = tokenize_multipart_input(
            #     input_text_list=eda_augmented_example,
            #     max_length=max_length,
            #     tokenizer=self.tokenizer,
            #     task_name=self.args.task_name,
            #     prompt=prompt,
            ...
            features = OurInputFeatures(inputs['input_ids'], inputs['attention_mask'], inputs['token_type_ids'], example_label, inputs['mask_pos'], None,
